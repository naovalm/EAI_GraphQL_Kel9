...
const dbConfig = {                   //define database
  host: 'localhost',
  user: 'root',
  password: '',
  database: 'holidays',
...
//define schema /data type, / input, query, dan mutasi 
const typeDefs = gql`  
  type Place {
    place_id: ID
    name: String
...
const resolvers = {                // Define Resolver
  Query: {
    places: (parent, args) => {
  ...
  Mutation: {
    createPlace: (parent, args) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        db.query(
          'INSERT INTO places (place_id, name, province, regency, category, price) VALUES (?, ?, ?, ?, ?, ?)',
          [place_id, name, province, regency, category, price],
          function (err) {
            if (err) {
              reject(err);
            } else {
    ...
    updatePlace: (parent, args) => {
      ...
      return new Promise((resolve, reject) => {
        db.query(
          'UPDATE places SET name = ?, province = ?, regency = ?, category = ?, price = ? WHERE place_id = ?',
          [input.name, input.province, input.regency, input.category, input.price, place_id],
          function (err) {
            if (err) {
              reject(err);
            } else {
